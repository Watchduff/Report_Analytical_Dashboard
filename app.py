...
Dynamic Excel upload -> detailed report analytics.
...
pal = PALETTES[theme_type]

def chart_theme(fig, **layout_kwargs):
...
st.markdown(
    ...
    .st-key-kpi_total [data-testid="stMetric"] {{ border-left: 3px solid {BYUH_RED} !important; }}
    .st-key-kpi_closed [data-testid="stMetric"] {{ border-left: 3px solid {pal["up"]} !important; }}
    .st-key-kpi_open [data-testid="stMetric"] {{ border-left: 3px solid {pal["warn"]} !important; }}
    .st-key-kpi_completion [data-testid="stMetric"] {{ border-left: 3px solid {BYUH_RED} !important; }}
    .st-key-kpi_categories [data-testid="stMetric"] {{ border-left: 3px solid {pal["muted"]} !important; }}
    ...
    /* KPI cards are clickable — jump to their tab. The button sits invisibly on top. */
...
# File upload
# -----------------------------
uploaded_file = st.file_uploader("Upload an Excel report (.xlsx)", type=["xlsx", "xls"])
...
# Normalize text fields so casing differences (e.g. "Hardware" vs "hardware")
# don't fragment the same category/status/technician into separate entries.
...
with tab_map["Overview"]:
    if has_status:
        ...
        with col2:
            ...
            chart_theme(fig_bar, showlegend=False)
...
if "Trends" in tab_map:
    with tab_map["Trends"]:
        if filtered_df["Modified"].notna().any():
            ...
            chart_theme(fig, xaxis_title="Date", yaxis_title="Ticket Count")
...
if "Categories" in tab_map:
    with tab_map["Categories"]:
        ...
        chart_theme(fig_cat, height=max(350, len(cat_counts) * 35))
        ...
        if has_status:
            ...
            chart_theme(fig_cross, height=max(350, len(cross) * 35))
            ...
            chart_theme(fig_comp, height=max(350, len(comp) * 35),
...
if "Technicians" in tab_map:
    with tab_map["Technicians"]:
        ...
        chart_theme(fig_tech, height=max(350, len(tech_counts) * 35), yaxis_title="Technician")
